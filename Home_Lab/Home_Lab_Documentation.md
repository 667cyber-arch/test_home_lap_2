...
```
  ...
  - *SC – database showing `paygearplanDB` and users in terminal*
...
- **Screenshot**
  ...
  - *SC – table `produktet` showing inventory data*
...
- *SC – table `users` showing user accounts and `created_at` timestamps*
...
- *SC – table `admins` showing admin usernames, roles, and privileges*
...
*SC – table `shporta` structure showing session fields*
...
[SC - Insert Nmap terminal execution screenshot]
...
[SC - Insert Gobuster scanning directories and files screenshots]
...
- Execution:
  ...
  * Simulated the discovery of a misconfigured server or exposed backup file (e.g., lidhjaDatabazes.php.bak).
...
[SC 4 - Capture the browser address bar with that filename. Caption: "CWE-522: Sensitive configuration file exposure. Discovered the database connection logic via targeted directory brute-forcing. also we successfully found the name of the database that this website it connected to "]
...
[SC - Insert Screenshot of the login page showing the "Incorrect credentials" alert]
...
[SC - Insert Screenshot of the Admin Dashboard or the echoed "Query tried" message here]
...
[SC - Insert Screenshot showing the users table data rendered on the product page]
...
[SC - Insert Screenshot showing the alert pop-up displaying the session cookie]
...
[SC - Insert Screenshot showing admin email and hashed password displayed on the product page]
...
[SC - Insert Screenshot showing the alert popup in the browser]
...
[SC - Insert Screenshot showing the alert popup on the product review page]
...
# Attack 12: Session Hijacking via Stored XSS
...
* Objective: Weaponize the Stored XSS vulnerability to extract administrative session cookies and perform complete account takeover.
...
- Concept:
  ...
  * Session hijacking occurs when an attacker steals a valid session identifier. By combining a Stored XSS vulnerability with an insecure cookie configuration, attackers can use JavaScript to extract the `document.cookie` object and transmit it to an external listening server.
...
* Security Impact: The weaponized payload was stored in the database. When an Administrator viewed the compromised page, the script executed silently in the background. The victim's `PHPSESSID` cookie was successfully extracted and transmitted to the attacker's remote listener. The attacker can now reuse this cookie to fully hijack the administrative session, bypassing all authentication mechanisms.



[SC - Insert Screenshot showing captured PHPSESSID cookie in attacker server logs]
